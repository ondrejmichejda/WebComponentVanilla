    ...
    <div class="calculator">
      <div class="outer-body">
        <div class="body">
          ...
          <div style="text-align: center"><a class="button" href="${this.url}">CHCEM ZAČÍT</a></div>
...
`,this.currencyChangeSubscription(),this.initSlider1(),this.initSlider2(),this.calculatePrice()}initSlider1(){var t=this.shadow.querySelectorAll("#slider1")[0];t&&t.noUiSlider&&t.noUiSlider.destroy(),n().create(t,{start:this.option.start,step:this.option.step,connect:[!0,!1],pips:{mode:"range",density:1e5,format:e()({decimals:0,thousand:this.option.format.thousand,prefix:this.option.format.prefix,suffix:this.option.format.suffix})},range:{min:[0],max:[this.option.max]}});const i=this.shadow.querySelectorAll("#slider1-value")[0];t.noUiSlider.on("update",(t,e)=>{let r=this.FormatCZK.from(t[e]);t=this.option.currency;r=("CZK"===t?this.FormatCZK:this.FormatEUR).to(r),i.innerHTML=r,this.calculatePrice()})}initSlider2(){var t=this.shadow.querySelectorAll("#slider2")[0];t&&t.noUiSlider&&t.noUiSlider.destroy(),n().create(t,{start:[30],step:1,connect:[!0,!1],pips:{mode:"range",density:1e3,format:e()({decimals:0,thousand:" ",suffix:" dnů"})},range:{min:[0],max:[90]}});const r=this.shadow.querySelectorAll("#slider2-value")[0];t.noUiSlider.on("update",(t,e)=>{t=parseInt(t[e]).toString().replace(/\B(?=(\d{3})+(?!\d))/g," ");r.innerHTML=t+" dnů",this.calculatePrice()})}calculatePrice(){var t=parseInt(this.shadow.querySelectorAll("#slider1-value")[0].textContent.replace(/\s+/g,"").replace(",","").replace("€",""));let e=.8*t,r=t/100*.0666*parseInt(this.shadow.querySelectorAll("#slider2-value")[0].textContent.replace(/\s+/g,"").replace(",","")),i=.2*t-r;r=("CZK"===this.option.currency?(e=this.FormatCZK.to(e),i=this.FormatCZK.to(i),this.FormatCZK):(e=this.FormatEUR.to(e),i=this.FormatEUR.to(i),this.FormatEUR)).to(r),this.shadow.querySelectorAll("#row1-value")[0].innerHTML=e,this.shadow.querySelectorAll("#row2-value")[0].innerHTML=i,this.shadow.querySelectorAll("#row3-value")[0].innerHTML=r}currencyChangeSubscription(){const t=this.shadow.querySelectorAll("#cur-czk")[0],e=this.shadow.querySelectorAll("#cur-eur")[0],r=this.shadow.querySelectorAll("#cur-span")[0];t.addEventListener("click",()=>{this.option!==this.optionCZK&&(e.classList.remove("active"),t.classList.add("active"),this.option=this.optionCZK,r.textContent=this.option.currency,this.initSlider1())}),e.addEventListener("click",()=>{this.option!==this.optionEUR&&(t.classList.remove("active"),e.classList.add("active"),this.option=this.optionEUR,r.textContent=this.option.currency,this.initSlider1())})}}customElements.define("transfactoring-calculator",r)})()})();